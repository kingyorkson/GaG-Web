...
-- Auth codes for mobile sign-in (replaces QR tokens)
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'auth_codes' AND policyname = 'authenticated_insert') THEN
    CREATE POLICY authenticated_insert ON auth_codes FOR INSERT WITH CHECK (auth.role() = 'authenticated');
  END IF;
END $$;
...
BEGIN
  ...
  WHERE q.code = code_text
    AND q.used = FALSE
    AND q.expires_at > NOW();

  IF result IS NOT NULL THEN
    UPDATE auth_codes SET used = TRUE WHERE code = code_text;
  END IF;
